...
This project builds the governance layer. Persona management that defines who the assistant is. Guardrails that control what it can and can't say. Intent detection that routes queries to the right workflow. Conversation history that remembers context. And edge case testing that proves it all works under pressure.

<!--more-->

## The Problem

In Part 1, our `PromptTemplateManager` was a Python class with `{variable}` substitution. No versioning, no persona, no safety controls. The `ContentValidator` redacted PII with regex — fine for a demo, but not how you'd protect a production system.

A real support assistant needs to know its role boundaries. It needs to refuse off-topic questions. It needs to detect when a customer is angry and escalate to a human. It needs to redact PII before it ever reaches the model. And you need to test all of this systematically, not just hope it works.
...
Denied topic: 🚫 BLOCKED
  "What's the competitor pricing for Azure?"
...
I'm getting a 403 error on my S3 bucket            troubleshooting    negative
How do I set up a VPC?                             how_to             neutral
How much does Lambda cost?                         billing            neutral
...
```
Turn 1:
  Customer: How do I set up IAM permissions for a Lambda function to access S3?
  Assistant: To set up IAM permissions for your Lambda function to access S3,
             follow these steps: 1. Create an IAM Role for Lambda...

Turn 2:
  Customer: What about DynamoDB access from the same Lambda?
  Assistant: To add DynamoDB access to the same Lambda function, you'll need
             to update the IAM role's policy...

Turn 3:
  Customer: Can you show me an example IAM policy for that?
  Assistant: Here's an example IAM policy that grants your Lambda function
             read and write access to a specific DynamoDB table...
```

Turn 2 understands "the same Lambda" because it has the conversation history. Turn 3 knows "that" refers to DynamoDB access. Without history, each turn would be a disconnected question.
...
- **Part 7** — Agentic AI with Bedrock Agents & Step Functions
...
In Part 7, we'll dive into agentic AI — building multi-agent systems with Bedrock Agents, the ReAct pattern, and human-in-the-loop workflows.
