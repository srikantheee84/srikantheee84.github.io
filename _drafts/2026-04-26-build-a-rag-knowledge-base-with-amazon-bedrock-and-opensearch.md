...
This project builds the infrastructure that makes RAG actually work. Real vector stores, real embeddings, real document processing — the production version of the Python dictionary we used in Part 1.
...
The difference shows up in retrieval quality. A query about "Lambda cold starts" should retrieve the paragraph that explains cold starts — not half of that paragraph and half of the next section about event sources.

```python
chunker = create_chunker("semantic", max_chunk_size=800)
chunks = chunker.chunk(text, document_id="aws-lambda-guide")
# 5 chunks, each a coherent section of the guide
```
...
Top match (score: 0.7589):
  "When Lambda runs your function, it creates an execution environment.
   The execution environment provides a secure and isolated runtime...
   Cold starts occur when Lambda needs to create a new execution
   environment. This adds latency to the first invocation."

Second match (score: 0.3127):
  "AWS Lambda is a serverless compute service that runs your code
   in response to events..."
```

The cold starts chunk scores 0.76. The general Lambda overview scores 0.31. Unrelated content (Bedrock, S3) scores below 0.12. The embedding model understands what the question is really asking.
...
🔄 CHANGE DETECTION (re-ingest)
   documents/amazon-s3-guide.txt: unchanged
   documents/aws-lambda-guide.txt: unchanged
   documents/amazon-bedrock-guide.txt: unchanged
```
...
```
Q: How do I minimize Lambda cold starts?
A: To minimize Lambda cold starts, you can use provisioned concurrency,
   which keeps a specified number of execution environments initialized
   and ready...
   Sources: 5 chunks, latency: 1322ms

Q: What vector stores does Bedrock Knowledge Bases support?
A: Bedrock Knowledge Bases support multiple vector store backends:
   Amazon OpenSearch Serverless, Amazon Aurora PostgreSQL (pgvector),
   Pinecone, Redis Enterprise, and MongoDB Atlas.
   Sources: 5 chunks, latency: 713ms

Q: How does S3 versioning protect against accidental deletions?
A: S3 versioning protects against accidental deletions by keeping
   multiple variants of an object in the same bucket. When enabled,
   S3 preserves every version of every object.
